function [y,min_distance,T,lunar_position,lunar_velocity] = LorbitRK4(dt,y0,lunar_posATinj)
    mu_lunar        =   4911.3;
    lunar_w         =   [0,0,2*pi / (27*24*3600)];     %phase2에서 달 공전속도 증분x
    y(:,1) = y0;
    i = 2;
    ...
    while true
        ...
        lunar_velocity(:,i) = cross(lunar_w,lunar_position(:,i))';  % 여기 고민

        pre_distance = distance;            %pre보다 멀어질때 끝나는게 아니라 24*3600*1인 이유?
